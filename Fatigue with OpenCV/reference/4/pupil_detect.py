...
def draw_circle_for_feature(image, feature, color=255, thickness=1):
    cv2.circle(image, (int(feature.pt[0]), int(feature.pt[1])), int(len(feature)/2), color, thickness)

def find_pupil(gray_image, minsize=.1, maxsize=.5):
    detector = cv2.MSER_create()
    features_all = detector.detectRegions(gray_image)
    features_big = [feature for feature in features_all if len(feature) > gray_image.shape[0]*minsize]
    features_small = [feature for feature in features_big if len(feature) < gray_image.shape[0]*maxsize]
    if len(features_small) == 0:
        return None
    features_sorted = sort_features_by_brightness(gray_image, features_small)
    pupil = features_sorted[0]
    return (int(pupil[0]), int(pupil[1]), int(len(pupil)/2))
